...
// represents fully on, valid duty cycles to pass in range from 0 to
// MAX_DUTY_CYCLE
...
int spot_pulse_state[NUM_SPOT]; // short term record of where solenoid is in PWM
                                // cycle
int spot_nozzle_state[NUM_SPOT]; // record of which spot solenoids are on/off

int lowtank_warnings = 0;
const int lowtank_limit =
    3; // need three low tank warnings before turning pump off.
...
void send_response(ResponseMessageStatus status, ResponseMessageSource source,
                   const char *message) {
  // confirm message is valid (no newlines allowed)
  const char *curr = message;
  while (*curr != '\0') {
    if (*curr == '\n') {
      send_response(ERROR, SYSTEM,
                    "invalid response attempted, message contains newline");
      return;
    }
    curr++;
  }
  ...
  Serial.println(); // must newline terminate messages so listener knows it's
                    // ended
}
...
void spot_command(BoomID boomID, unsigned int nozzle, unsigned int duty) {
  if (!(boomID == CENTER // only support center boom for now
...
void increment_pwm_nozzles() {
  ...
  for (int n = 0; n < NUM_SPOT; n++) {
    if (spot_nozzle_state[n] != 0) {
      if (spot_pulse_state[n] == 1 &&
          (currentTime - previousTime[n] >= high_time[n])) {
        spot_pulse_state[n] = 0;
        previousTime[n] = currentTime;
        mcp.digitalWrite(n, LOW);
      } else if (spot_pulse_state[n] == 0 &&
                 (currentTime - previousTime[n] >= low_time[n])) {
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (bytesRead > 1 && buffer[bytesRead - 1] == '\r') {
      send_response(
          STATUS, SYSTEM,
          "Received CRLF line ending, handling but should be avoided");
      buffer[--bytesRead] = '\0';
    }
