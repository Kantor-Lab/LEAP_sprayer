...
def generate_launch_description():
    ...
    declare_zenoh = DeclareLaunchArgument(
        'zenoh', default_value='false', description='Launch zenoh router'
    )
    ...
    robot_state_publisher = Node(
        ...
        output='both',
        parameters=[{'use_sim_time': False}, {'robot_description': robot_urdf}],
    )

    declare_rviz = DeclareLaunchArgument('rviz', default_value='true', description='Open RViz.')
    ...
    launch_actions = [robot_state_publisher, declare_rviz, rviz]

    start_with_zenoh = RegisterEventHandler(
        OnProcessStart(target_action=zenoh_start, on_start=launch_actions),
        condition=IfCondition(use_zenoh),
    )

    start_without_zenoh = GroupAction(actions=launch_actions, condition=UnlessCondition(use_zenoh))
