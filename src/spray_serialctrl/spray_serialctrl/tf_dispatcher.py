...
def get_nozzle_box_intersections(
    ...
    boxes_adjacent = boxes_adjacent.squeeze(
        axis=1
    )  # sliding windows adds 1 dimension to shape iterating nothing on the 6-elem axis
    assert (
        boxes_adjacent.shape[0] == boxes_nozzles.shape[0] - 1
        and boxes_adjacent.shape[1] == 2
        and boxes_adjacent.shape[2] == 6
    ), f'Invalid shape {boxes_adjacent.shape = }'
    ...
    np.subtract(
        ...
        out=boxes_nozzles[
            :, 4
        ],  # send it straight into boxes_nozzles width, no unnecessary copying
    )
...
class NozzleCommandDispatcher(Node):
    ...
    def get_nozzle_boxes(self) -> AlignedBoundingBoxArray:
        ...
        nozzle_frame_name = (
            r'spot_nozzle\d'  # need to change to \d+ to support more than 10 nozzles
        )
        ...
        # may throw
        nozzle_to_baselink_transforms = [
            self.tf_buffer.lookup_transform('odom', nozzle_frame, tf2_ros.Time())
            for nozzle_frame in nozzle_frames
        ]
    ...
    def listener_callback(self, msg: Detection3DArray):
        ...
        boxes_all: list[None | BoundingBox3D] = [None] * (
            boxes_unique.shape[0] + boxes_intersect.shape[0]
        )
        ...
        self.spray_box_publisher.publish(
            SceneUpdate(
                deletions=[SceneEntityDeletion(timestamp=stamp, type=SceneEntityDeletion.ALL)],
                entities=[
                    SceneEntity(
                        timestamp=stamp,
                        frame_id='odom',
                        id=f'uniquesprayerbox_{i // 2}'
                        if i % 2 == 0
                        else f'intersectionsprayerbox_{i // 2}_{i // 2 + 1}',
                        lines=[
                            bbox_to_line(
        ...
        for detection in msg.detections:  # grab all boxes
            ...
            for i, box_intersect in enumerate(boxes_all[1::2]):
                assert box_intersect is not None, (
                    f'Invalid box at index {i * 2 + 1} in boxes_intersect'
                )
                if boxes_overlap(
                    detection.bbox, box_intersect, 0
                ):  # buffer must match unique to minimize flickering
        ...
        for intersect in intersect_list:
            assert intersect < len(fboom_new) - 1, (
                f'Invalid intersect at index {intersect} given {len(fboom_new)} nozzles'
            )
